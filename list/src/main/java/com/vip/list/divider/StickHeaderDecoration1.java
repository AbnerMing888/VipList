...
public class StickHeaderDecoration1 extends RecyclerView.ItemDecoration {
    ...
    public StickHeaderDecoration1(Context context) {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        if (parent.getAdapter() instanceof BAdapter) {
            BAdapter adapter = (BAdapter) parent.getAdapter();
            int count = parent.getChildCount();//获取可见范围内Item的总数
            for (int i = 0; i < count; i++) {
                View view = parent.getChildAt(i);
                int position = parent.getChildLayoutPosition(view);
                boolean isHeader = adapter.isItemStick(position);
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        if (parent.getAdapter() instanceof BAdapter) {
            ...
            boolean isHeader = adapter.isItemStick(position + 1);
